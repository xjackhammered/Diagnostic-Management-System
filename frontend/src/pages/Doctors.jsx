...
const empty = { name: '', age: '', contact_number: '', email: '', gender: '', specialization: '' }
...
export default function Doctors() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { invalidate(); setModal(null); setForm(empty); toast.success('Doctor added') },
    onError: () => toast.error('Failed'),
  })

  const updateMut = useMutation({
    mutationFn: ({ id, data }) => updateDoctor(id, data),
    onSuccess: () => { invalidate(); setModal(null); toast.success('Updated') },
    onError: () => toast.error('Failed'),
  })
  ...
  const handleSubmit = () =>
    modal === 'create' ? createMut.mutate(form) : updateMut.mutate({ id: editing.id, data: form })
  ...
  return (
    <div>
                  ...
                  <tr key={d.id}>
                    ...
                    <td>
                      <div className="flex gap-2">
                        ...
                        <button className="btn btn-sm btn-danger" onClick={() => deleteMut.mutate(d.id)}>
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
      ...
      {modal && (
        ...
        >
          {/* Inlined form – no extra component, so inputs keep focus */}
